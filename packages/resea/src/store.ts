...
export const createSea = (): SeaType => {
  ...
  const actions = new Set<(event: SeaActionEvent) => void>();
...
>($id: Id, options: DefineStoreOptions<T, G, A> = {}): DefineStoreType<Id, T, G, A> {
  ...
  let realStore: SeaStore<T> & T & G & A & { $state: T; $id: Id } | undefined;
  const createRealStore = (): typeof realStore => {
    ...
    if (reSea.stores.has($id)) {
      realStore = reSea.stores.get($id)! as SeaStore<T> & T & G & A & { $state: T; $id: Id };
      return realStore;
    }
    ...
    const baseStore: SeaStore<T> = {
      ...
      $onAction(callback: (event: SeaActionEvent) => void): () => void {
        ...
        reSea.actions.add(wrapped);
        return () => reSea.actions.delete(wrapped);
      },
      /**
  ...
  useStore.$getStore = createRealStore;
  useStore.$id = $id;
  return useStore as DefineStoreType<Id, T, G, A>;
}
